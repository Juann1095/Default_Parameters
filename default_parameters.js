...
What that means is, i´ll use an ES6 feature which allows me to set a default parameter inside a function definition, which goes hand in hand with the defensive coding approach,
...
/*Obviously, the --noDefaultParams-- function should return whatever number it recieves, squared.
...
arguments, while still being flexible enough to allow me to pass custom argument values and deal with them
accordingly.
...
Obviously, the --bool1-- should be set to --true-- on invocation to make this work, but i´ll set it to false on purpose, 
to highlight the point i´m making.
*/
...
/* This approach improves the developer experience of my code, bacause I no longer have to worry about feeding
the --WithDefaultParameters-- class with all the arguments. For quick tests this is great, because i no longer need
to worry about passing the proper arguments.

additionally, this approach really shines when building inheritance hierarchies using classes, as it makes it possible
to provide only the custom properties in the sub-class, while still accepting all the default parameters 
from the super-class constructor.

This exercise i´ve covered the following
-How to approach designing an object-oriented program in JavaScript
-The role of the --extends-- and --super-- keyword
